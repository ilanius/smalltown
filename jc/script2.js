...
/*
/* ********************************************************** */
function httpPost2( fDat, callBack ) {
    var xhr = new XMLHttpRequest();
    xhr.onreadystatechange = function() {
      if (this.readyState == 4 && this.status == 200) {
          if ( callBack) callBack( xhr.responseText );   
...
function uploadProfileImage( e ) {
    ...
    if ( file.name.match( /\.(png|jpg|jpeg|gif)$/ ) ) {
        ...
        fDat.append('func',    'userAccount' );
        fDat.append('subFunc', 'update' );
        fDat.append('image',   file );
        httpPost2( fDat, reloadPage ); // this does not work for some reason: location.reload );
    }
}
...
function postImageSubmit( e, o ) {
    ...
    if ( file.name.match( /\.(png|jpg|jpeg|gif)$/ ) ) {
        var pId = o.name.substring( 'commentInput'.length );  // pId will be parent of this post
        let fDat = new FormData();
        fDat.append('func',    'postImageSubmit' );
        fDat.append('profileId', profileId );
        fDat.append('ppId', pId );
        fDat.append('lastFeedTime', lastFeedTime );
        fDat.append('pTxt', o.value );
        fDat.append('image',   file );
        if ( pId.length > 0 ) {
            gid( 'commentInput'+pId).remove(); // text input not post is removed 
        }     
        resetFeedUpdate(); 
        httpPost2( fDat, feedUpdateSet ); // this does not work for some reason: location.reload );
    }
    return deLight( o.id );
}
...
function deLight( id) {
    let o = gid( id );
    if ( o ) o.classList.remove('highLight');
    return false;
}
...
function addPostSubmitField( event, pId ) {
    ...
    let id = `commentInput${pId}`;
    comment.innerHTML = `<input type="text" ondragover="return highLight('${id}');" ondragleave="return deLight('${id}');" ondrop="return postImageSubmit(event, this);" id="${id}" name="${id}"` + 
